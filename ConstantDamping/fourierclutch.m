...
% Dynamics, currently don't consider u1, instead generator assumed a constant damping C = 50
ceq = [(Phi*Dphi*x2*500 + 1500*Phi*x2 + 2000*(Phi*Iphi*x2) - T - Ff*((u2p-u2q).*(Phi*x3+x3mean)-(u2p+u2q).*(Phi*x2)));
    (Phi*Dphi*x3*100 + (50*(Phi*x3 + x3mean)) + Ff*((u2p+u2q).*(Phi*x3+x3mean)-(u2p-u2q).*(Phi*x2)));
    u2p.*u2q];
...
if nargout > 2
    ...
    gradceq = [Phi*Dphi*500 + 1500*Phi + 2000*Phi*Iphi + Ff*(u2p+u2q).*Phi,...
        ...
        -Ff*(u2p-u2q).*Phi, Phi*Dphi*100 + 50*Phi + Ff*(u2p+u2q).*Phi, diag(Ff*(Phi*x3+x3mean)-Ff*(Phi*x2)),...
        diag(Ff*(Phi*x3+x3mean)+ Ff*(Phi*x2)), Ff*(u2p+u2q)+ 50;
